    ...
    <article className="card-pad space-y-3">
      <header className="flex items-start justify-between gap-2.5">
        <div className="min-w-0 flex-1">
          {showRoute ? (
            <Link to={`/routes/${arrival.routeId}`} className="flex flex-wrap items-center gap-1.5 sm:gap-2">
              <span className="rounded-lg bg-brand-700 px-2 py-0.5 text-xs sm:text-sm font-bold text-white">
                {arrival.routeCode}
              </span>
              <span className="truncate font-semibold text-slate-800 text-sm sm:text-base">{arrival.routeName}</span>
            </Link>
          ) : (
            <span className="font-semibold text-slate-800 text-sm sm:text-base">{arrival.routeName}</span>
          )}
        ...
        <div className="shrink-0 text-right pl-1">
          {show && eta ? (
            <>
              <p className="text-xl sm:text-2xl font-bold leading-none text-slate-900">{etaText(eta)}</p>
              <p className="text-[10px] sm:text-xs font-semibold uppercase tracking-wide text-slate-500 mt-0.5">
                estimated
              </p>
            </>
          ) : (
            <p className="text-xs sm:text-sm font-semibold text-warn">No estimate</p>
          )}
        </div>
      </header>
      ...
      {show && eta ? (
        ...
        <p className="rounded-xl bg-warn-soft px-3 py-2 text-xs sm:text-sm text-warn">
          {ETA_UNAVAILABLE_TEXT[reason] ?? 'No estimate is available for this bus yet.'}
        </p>
      )}

      <footer className="flex flex-wrap items-center gap-1.5 sm:gap-2 border-t border-slate-100 pt-3">
        ...
        <Link
          ...
          className="ml-auto inline-flex items-center py-1 text-xs font-semibold text-brand-700 hover:underline min-h-[32px]"
        >
          Trip detail →
        </Link>
      </footer>
    ...
    <article className="card-pad space-y-2">
      <header className="flex items-start justify-between gap-2.5">
        ...
        <div className="shrink-0 text-right pl-1">
          {show && next ? (
            <>
              <p className="text-lg sm:text-xl font-bold leading-none">{etaText(next)}</p>
              <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5">est. {rangeText(next)}</p>
            </>
          ) : (
            <p className="text-xs sm:text-sm font-semibold text-warn">No estimate</p>
          )}
